...
def test_get_token(client, user):
    cred = dict(username=user.name, password='geheim')
...
def test_get_token_admin(client, user, admin_user):
    get_json(client, 'annotator/users/new@example.com/token', user=user, params=dict(username=user.name), expected=401)
    result = get_json(client, f'annotator/users/{user.name}/token', user=admin_user, params=dict(user=user.name), expected=200)
    assert auth.verify_token(result['token']) == user
...
def test_job_users(client, admin_user, user):
    ...
    post_json(client, f'annotator/codingjob/{jobid}/users', json={'users': [user.name]}, user=admin_user, expected=204)
    get_json(client, f'annotator/codingjob/{jobid}/unit', user=user, expected=200)
    # Can we add users as part of the rules?
    jobid = post_json(client, 'annotator/codingjob', user=admin_user,
                      json={'authorization': {'restricted': True, 'users': [user.name]}, **job_dict})['id']
    get_json(client, f'annotator/codingjob/{jobid}/unit', user=user, expected=200)


def test_job_tokens(client, job, admin_user):
    t = get_json(client, f'annotator/codingjob/{job}/token', user=admin_user)
    assert set(get_json(client, f'annotator/guest/jobtoken', params=dict(token=t['token'])).keys()) == {"job_id", "name", "token", "is_admin"}
    result = get_json(client, f'annotator/guest/jobtoken', params=dict(token=t['token'], user_id='pietje'))
    assert 'pietje' in result['name']
